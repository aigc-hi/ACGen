    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*training_models):
                ...
                # 读取 batch 中的文件路径
                image_paths = batch["image_paths"]  # 训练时的批次图像路径

                # 提取角色 ID（去掉角度信息）
                character_ids = [path.split("_")[1] for path in image_paths]  # 假设文件名格式：character_001_front_001.png
                angle_labels = [path.split("_")[2] for path in image_paths]  # 角度部分
                ...
                for i in range(len(character_ids)):
                    for j in range(i + 1, len(character_ids)):
                        # 确保是同一个角色但不同角度
                        if character_ids[i] == character_ids[j] and angle_labels[i] != angle_labels[j]:
                            kl_loss = F.kl_div(F.log_softmax(model_pred[i], dim=-1), 
                                               F.softmax(model_pred[j], dim=-1), 
                                               reduction="batchmean")
                ...
                if count > 0:
                    ...
                    loss += consistency_loss * 0.1  # 调整 0.1 权重
